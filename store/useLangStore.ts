...
export const useLangStore = defineStore("lang", {
  ...
  getters: {
    ...
    currentLocaleConfig: (state): LocaleConfig => {
      return (
        state.availableLocales.find((l) => l.code === state.currentLocale)! ||
  ...
  actions: {
    ...
    async setLocale(locale: Locale) {
      ...
      if (!this.isLocaleAvailable(locale)) {
        console.warn(
          `Locale "${locale}" non disponible, utilisation du fallback`
        );
    ...
    saveToCookie(locale: Locale) {
      if (process.client) {
        ...
        expirationDate.setDate(
          expirationDate.getDate() + this.cookieExpiration
        );
      ...
      if (process.client) {
        ...
        if (saved) {
          try {
            ...
            console.error(
              "Erreur lors du chargement des préférences de langue",
              error
            );
          }
        }
    ...
    /**
     * Formate une date en fonction de la locale actuelle
     */
    formatDate(
      date: Date | string,
      ...
      if (!(dateObj instanceof Date) || isNaN(dateObj.getTime())) {
        console.warn("Date invalide passée à formatDate:", date);
        return "";
      }

      // 🧹 Nettoyer les options pour éviter les proxies Vue
      const safeOptions =
      ...
      try {
        return new Intl.DateTimeFormat(
          this.currentLocaleISO,
          defaultOptions
        ).format(dateObj);
      } catch (error) {
        console.error("Erreur formatDate:", error, { options: defaultOptions });
        return dateObj.toLocaleDateString(this.currentLocaleISO);
      }
    },
